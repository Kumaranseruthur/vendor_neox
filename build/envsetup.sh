...
cat <<EOF
Additional NeoxROM functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- lineagegerrit:   A Git wrapper that fetches/pushes patch from/to NeoxROM Gerrit Review.
- lineagerebase:   Rebase a Gerrit change and push it again.
- lineageremote:   Add git remote for NeoxROM Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for NeoxROM Github.
...
function breakfast()
{
    target=$1
    local variant=$2
    NEOX_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/neox/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the neox model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch neox_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/neox-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.neox.device | grep -q "$NEOX_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $NEOX_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function lineageremote()
{
    ...
    local NEOX_USER=$(git config --get review.review.lineageos.org.username)
    if [ -z "$NEOX_USER" ]
    then
        git remote add lineage ssh://review.lineageos.org:29418/$PFX$PROJECT
    else
        git remote add lineage ssh://$NEOX_USER@review.lineageos.org:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/NeoxROM/$PROJECT
...
function installboot()
{
    ...
    if (adb shell getprop ro.neox.device | grep -q "$NEOX_BUILD");
    then
...
function lineagegerrit() {
    ...
    local user=`git config --get review.review.neoxos.org.username`
...
function lineagerebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "NeoxROM Gerrit Rebase Usage: "
...
function dopush()
{
    ...
    if (adb shell getprop ro.neox.device | grep -q "$NEOX_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/neox/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $NEOX_FIXUP_COMMON_OUT ]; then
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/neox/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
