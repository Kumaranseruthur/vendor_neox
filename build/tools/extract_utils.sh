...
function setup_vendor() {
    ...
    export NEOX_ROOT="$3"
    if [ ! -d "$NEOX_ROOT" ]; then
        echo "\$NEOX_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$NEOX_ROOT/$OUTDIR" ]; then
        mkdir -p "$NEOX_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$NEOX_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$NEOX_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$NEOX_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$NEOX_ROOT"/vendor/neox/build/tools/smali/baksmali.jar
        export SMALIJAR="$NEOX_ROOT"/vendor/neox/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$NEOX_ROOT"/vendor/neox/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$NEOX_ROOT"/vendor/neox/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$NEOX_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$NEOX_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$NEOX_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$NEOX_ROOT"/vendor/neox/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$NEOX_ROOT"/"$OUTDIR"/radio
