...
func init() {
	android.RegisterModuleType("neox_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = neoxExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = neoxExpandVariables(ctx, dep_file)
	...
	cmd := neoxExpandVariables(ctx, String(g.properties.Cmd))
